...
function loadCandidateCareerInformationData() {
    ...
    if($('#addEducationModal').length) {
        ...
        $('#educationStateId').select2({
            'width': '100%',
            'placeholder': Lang.get('messages.company.select_state'),
            dropdownParent: $('#addEducationModal')
        });
        $('#educationCityId').select2({
            'width': '100%',
            'placeholder': Lang.get('messages.company.select_city'),
            dropdownParent: $('#addEducationModal')
        });
    }
    ...
    if ($('#addExperienceModal').length) {
        ...
        $('#cityId').select2({'width': '100%', 'placeholder': Lang.get('messages.company.select_city'), dropdownParent: $('#addExperienceModal')});
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    $('#editCountry').val(result.data.country_id).
                        trigger('change', [
                            {
    ...
    listenHiddenBsModal('#addEducationModal', function () {
        ...
        $('#degreeLevelId').val('');
        $('#degreeLevelId').select2({'width': '100%', 'placeholder': Lang.get('messages.company.select_state')});
        $('#educationYearId').val('');
        $('#educationYearId').select2({
            'width': '100%',
            'placeholder': Lang.get('messages.candidate_profile.select_year')
        });
    ...
    listenClick('.delete-experience', function (event) {
        let experienceId = $(event.currentTarget).data('id');
        deleteItem(route('experience.destroy', experienceId), Lang.get('messages.candidate_profile.experience'),
    ...
    listenClick('.delete-education', function (event) {
        let educationId = $(event.currentTarget).data('id');
        deleteItem(route('candidate.update-education', educationId), Lang.get('messages.candidate_profile.education'),
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.data.length != 0) {
                ...
                $(state).append(
                    $('<option value=""></option>').text(Lang.get('messages.company.select_state')));
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.data.length != 0) {
                ...
                $(city).append(
                    $('<option value=""></option>').text(Lang.get('messages.company.select_city')));
            }
        },
    });
}
